...
import { AccountVToken, Account, AccountVTokenTransaction, Comptroller, Market } from '../types/schema'
...
export function updateAccountLiquidity(
  ...
  const liquidityResult = comptrollerContract.getAccountLiquidity(accountAddress);
  log.debug('[updateAccountLiquidity] fetched data - err: {}, liq: {}, shortfall: {}, account: {}',
    [
      liquidityResult.value0.toString(), 
      liquidityResult.value1.toString(), 
      liquidityResult.value2.toString(), 
      accountAddress.toHexString()
    ]
  );
  if (liquidityResult.value0.equals(BigInt.zero())) {
    account.liquidity = [
      new BigDecimal(liquidityResult.value0), 
      new BigDecimal(liquidityResult.value1), 
      new BigDecimal(liquidityResult.value2)
    ];
    account.save();
  }
}
...
export function createAccount(accountID: string): Account {
  ...
  updateAccountLiquidity(Address.fromString(accountID));
  account.save()
  return account
}
...
export function ensureComptrollerSynced(
  ...
  log.debug('[ensureComptrollerSynced] comptroller info completed, oracle: {} closeFactor: {} liquidationIncentive: {} maxAssets: {}', [
